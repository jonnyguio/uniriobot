module.exports = {
    "erro": "Foi mal, não entendi o que você quis dizer. Nenhuma dessas palavras é uma palavra-chave!",
    "obrigado": "De nada!",
    "ajuda": [
        "Abaixo estão as minhas funcionalidades atuais. Para utilizá-las, basta digitar o que está em colchetes. Eu entendo algumas variações, mas não força a barra (eu sou esperto mas nem tanto!).",
        "[WiFi]: mostra todas as senhas de wi-fi públicos.\n\t[Período]: informações sobre datas dos períodos desse ano: início e término, trancamento, inscrição...\n\t[Calendario]: datas do Calendário Acadêmico de 2017.\n\t[TrancamentoDisciplinas]: informações variadas sobre o Trancamento de Disciplinas.\n\t[InscricaoDisciplinas]: informações variadas sobre a Inscrição em Disciplinas.\n\t[BilheteUnico]: procedimentos de como cadastrar seu Bilhete Único Universitário.",
        "Teve alguma ideia de algo que pode ser melhorado? Envie sua sugestão digitando [Sugestao], que nós vamos recebê-la diretamente e tratá-la com carinho! Pshhhaw."
    ],
    "wifi": [
        "Woosh! Esta é a lista de todas as senhas de wi-fi públicas. Dá uma sacada no que você quer!",
        "hack@unirio: ccet2017",
        "HACK@UNIRIO: usuário hack_unirio, senha ccet2016"
    ],
    "sugestoes": "Vuash! Envie-nos uma mensagem com sua sugestão. Nós receberemos ela com carinho e vamos trabalhar em cima disso.",
    "periodo": {
        "message": "Que tipo de informação você deseja?",
         "button": [
            "Calendário Acadêmico",
            "Trancamento de Disciplinas",
            "Inscrição em Disciplinas"
         ]
    },
    "calendario": [
        "Primeiro Período:\nPara alunos de graduação em geral, de 06/03 a 08/07. Para a Faculdade de Arquitetura de Urbanismo e da Escola de Belas Artes, de 31/02 a 08/07",
        "Segundo Período:\nPara alunos de graduação em geral, incluindo os cursos da Faculdade de Arquitetura e Urbanismo e da Escola de Belas Artes de 31/07 a 09/12",
    ],
    "trancamento": [
        "Bzzzom. O período de trancamento de disciplinas de 2017/1 vai de 20/03 a 31/03! Para trancar disciplinas, você precisa realizar os seguintes procedimentos:",
        "* Se logue no SIE e vá na aba ‘Grade Curricular -> Trancamento de Disciplinas’. Lá você poderá realizar todas as ações importantes quanto ao trancamento.",
        "Caso o período de trancamento tenha se encerrado, converse com o coordenador do seu curso! Pelo que estou vendo aqui, o coordenador do seu curso é o Prof. Paulo - a sala dele é a F2015. Dá uma passada lá!"
    ],
    "inscricao": [
        "Bshawww. O período de inscrição em disciplinas de 2017/1 vai de 29/02 a 06/03! Para trancar disciplinas, você precisa realizar os seguintes procedimentos:",
        "* Se logue no SIE e vá na aba ‘Grade Curricular -> Inscrição em Disciplinas. Lá você poderá realizar todas as ações importantes quanto ao trancamento.",
        "Caso o período de inscrição tenha se encerrado, converse com o coordenador do seu curso! Pelo que estou vendo aqui, o coordenador do seu curso é o Prof. Paulo - a sala dele é a F2015. Dá uma passada lá!",
    ],
    "bilhete-unico": {
        "message": "O que deseja saber quanto ao Bilhete Único Universitário?",
        "button": {
            "label": "Cadastramento",
            "postback": "button-cadastramento",
            "postback-message": [
                "Para cadastrar seu bilhete único universitário, você precisa de diversos documentos. Veja em mais detalhes em https://www.cartaoriocard.com.br/rcc/gratuidade/legislacaopasselivre.",
                "Os documentos que a universidade pode te fornecer são:\n* Comprovante de Matrícula\n* Certificação de Previsão de Conclusão de Curso",
                "Estes dois documentos podem ser obtidos no SIE. Eles devem ser impressos por você e levados na secretaria: peça a um funcionário para assinar, carimbar e timbrar os seus documentos. Não se esqueça de pedir para timbrar! É o carimbo de alto-relevo (sem ele, podem decidir encrencar com você na Central do Riocard).",
            ]
        }
    },
    "aguardando": "Calma aí, deixa eu checar minhas planilhas....",
}